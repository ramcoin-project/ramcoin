...
#if defined(NDEBUG)
# error "Ramcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd23a1f7a1b33804705e044709727c6ff47ba3aba66b3ccb38e73820e78a4e094");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ramcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ramcoin Signed Message:\n";
...
{
    // Ramcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Ramcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Ramcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Ramcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ramcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ramcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3e02a7c0d663406dfb72ee33d6337c89ff121ca7a38394810d47fdcdb65635e3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Ramcoin 31/Dec/2018 When will humans be able to colonize the solar system?";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049da82bd4dc0a8fa426780a20b7dc8339b2a01273cdad2bef289bb70c51bc90a364f10992507d727e15de7c1e3418b1b00821ee37680c2a8b7202ee983b5f63e5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1539936344;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085074177;

        if (fTestNet)
        {
            block.nTime    = 1539936296;
            block.nNonce   = 386183821;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xa2cfacc62df9923fa73cdd1ed13820a5603801b8ca86f9fc4c25d18f22395ab5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xc7, 0xb7, 0xa9, 0xf8 }; // Ramcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RamcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RamcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RamcoinMiner(CWallet *pwallet)
{
    printf("RamcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ramcoin-miner");
    ...
    try { loop {
        ...
        printf("Running RamcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RamcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RamcoinMiner, pwallet));
}
